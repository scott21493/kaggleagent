...
| `tests/test_budget_waste.py` | observe_failed_command tracks repeats; check_task_caps raises on `repeated_same_failure_per_task > ceiling` |
...
| `tests/test_controller_watchdog.py` | wrap_invoke calls governor pre/post; raises if kill switch active; raises if usage_proxy exceeds caps |
...
| `arena/scoreboard/store.py` | Extend `insert_experiment` to take usage_proxy fields; add `get_run_usage_totals(competition_slug)` |
...
def test_check_task_caps_raises_on_repeated_failure_breaker() -> None:
    detector = WasteDetector(Phase0HardCeilings())
...
Expected: ImportError (waste module + governor's BudgetExceeded missing).
...
- [ ] **Step 4: Verify the test still fails (governor.BudgetExceeded missing)**

The waste module imports work, but `BudgetExceeded` is referenced lazily and the tests will fail when that path executes. Confirm:

```bash
.venv/Scripts/python.exe -m pytest tests/test_budget_waste.py -v
```

Expected: 4 of 5 tests pass; `test_check_task_caps_raises_on_repeated_failure_breaker` fails with ImportError on `arena.budget.governor`.

This is correct TDD ordering — Task 4 will land governor and unblock the last test. Continue to Task 4 without committing yet.

- [ ] **Step 5: Defer commit until Task 4 lands governor**

Do not commit. Move to Task 4 — the waste tests will turn green there.
...
def test_record_post_invoke_raises_on_input_chars_total() -> None:
    ...
    assert exc.value.breaker is Breaker.WALL_CLOCK or exc.value.breaker is Breaker.PROVIDER_CALL
    # Note: there is no dedicated "input chars breaker" in the 5 PR2 owns.
    # Map this overflow to ProviderCallBreaker as a coarse proxy in PR2;
    # PR4 may introduce a dedicated breaker.
...
(Note: the `test_record_post_invoke_raises_on_input_chars_total` accepts either `WALL_CLOCK` or `PROVIDER_CALL` because PR2 owns 5 breakers and there is no dedicated input-chars breaker. The implementation maps total-chars overflow to `PROVIDER_CALL` as the closest proxy. PR4 can introduce a dedicated breaker if needed.)
...
Expected: governor 10 passed; waste 5 passed (the deferred one from Task 3 now passes).
...
Expected: 76 tests total (was 62; +9 kill switch + 5 waste + ... actual count: 62 + 9 + 5 + 10 = 86 ... will verify in run). All checks clean.

(Note: exact count depends on parametrized cases. Adjust when running.)

- [ ] **Step 6: Commit waste + governor together**

```bash
git add arena/budget/waste.py arena/budget/governor.py \
        tests/test_budget_waste.py tests/test_budget_governor.py
git commit -m "$(cat <<'EOF'
feat(budget): WasteDetector + BudgetGovernor with cap enforcement

WasteDetector tracks per-task repeated-same-failure (the live event
observer that calls observe_failed_command lands in PR4; PR2 ships the
data shape and check_task_caps).
...
def test_wrap_invoke_raises_when_kill_switch_file_present(
    ...
    with pytest.raises(KillSwitchActive):
        watchdog.wrap_invoke(_MockProvider(), _packet())
    # provider was NOT called.
    assert governor.accumulators.provider_calls == 0


def test_wrap_invoke_raises_when_kill_switch_env_var(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv(KILL_SWITCH_ENV, "1")
    monkeypatch.chdir(tmp_path)
    governor = BudgetGovernor(Phase0HardCeilings())
    watchdog = Watchdog(governor=governor)
...
def test_wrap_invoke_raises_shell_command_breaker_on_misbehaving_provider(
    ...
    command events trips ShellCommandBreaker."""
    monkeypatch.delenv(KILL_SWITCH_ENV, raising=False)
    monkeypatch.chdir(tmp_path)
    governor = BudgetGovernor(Phase0HardCeilings())
    watchdog = Watchdog(governor=governor)
...
def test_wrap_invoke_pre_invoke_blocks_thirteenth_call(tmp_path: Path, monkeypatch) -> None:
    ...
    with pytest.raises(BudgetExceeded) as exc:
        watchdog.wrap_invoke(_MockProvider(), _packet())
    assert exc.value.breaker is Breaker.PROVIDER_CALL
    # provider was NOT called — pre-invoke check fired first.
```
...
class KillSwitchActive(Exception):
    """Raised when KillSwitch.is_active() is true at the start of wrap_invoke.

    Distinct from BudgetExceeded because the kill switch is a manual stop,
    not a cap violation. The CLI translates this into a BLOCKED_KILL_SWITCH
    phase rather than BLOCKED_BUDGET.
    """
...
class Watchdog:
    ...
    Sequence:
      1. KillSwitch.is_active() → raise KillSwitchActive (do not invoke).
      2. governor.check_pre_invoke(provider.name) → raise BudgetExceeded.
      3. provider.invoke(packet) → ProviderResult.
      4. governor.record_post_invoke(provider.name, result.usage_proxy,
         task_id=...) → raise BudgetExceeded.
      5. Return result.
    ...
    def wrap_invoke(self, adapter: ProviderAdapter, packet: dict) -> ProviderResult:
        if self._kill_switch.is_active():
            raise KillSwitchActive(
                f"kill switch active; refusing to invoke {adapter.name!r} for "
                f"task {packet.get('task_id', '<unknown>')}"
            )
        self._governor.check_pre_invoke(adapter.name)
...
git commit -m "$(cat <<'EOF'
feat(controller): Watchdog wires kill switch + budget governor around invokes

Watchdog.wrap_invoke checks KillSwitch.is_active first (raises
KillSwitchActive), then governor.check_pre_invoke (raises BudgetExceeded
on call-count overflow), invokes the provider, and finally
governor.record_post_invoke validates the returned UsageProxy against
per-task and per-run ceilings.

KillSwitchActive is a separate exception from BudgetExceeded because the
kill switch is a manual stop, not a cap violation; the CLI will translate
each into the right Phase (BLOCKED_KILL_SWITCH vs BLOCKED_BUDGET) in the
next task.
...
```python
    def get_run_usage_totals(self, competition_slug: str) -> dict[str, Any]:
        """Sum the usage_proxy fields across all experiments for the latest
        run of `competition_slug`. Used to seed `BudgetGovernor.accumulators`
        and to drive `arena budget status`.

        Returns 0/0.0 for every field when there are no experiments yet.
        """
        conn = self._require_conn()
        # latest run for the slug = max created_at among experiments.
        row = conn.execute(
            "SELECT "
            ...
            " FROM experiments WHERE competition_slug = ?",
            (competition_slug,),
        ).fetchone()
...
def test_get_run_usage_totals_sums_across_experiments(tmp_path: Path) -> None:
    store = ScoreboardStore(tmp_path / "s.sqlite")
    store.connect()
    ...
    totals = store.get_run_usage_totals("tabular_binary_v1")
...
def budget_status(
    ...
    totals = _store().get_run_usage_totals(slug)
...
get_run_usage_totals(slug) which SUMs across all experiments for a slug.
Both changes are additive — existing callers don't need to update.
...
- [ ] **Step 1: Wrap the provider invoke in run_next with the Watchdog**

In `arena/cli.py`, replace the existing `run_next` body with the version below. The changes:

1. Construct `BudgetGovernor` seeded from scoreboard usage totals.
2. Construct `Watchdog(governor=...)`.
3. Replace `result = adapter.invoke(packet)` with `result = watchdog.wrap_invoke(adapter, packet)`.
4. Catch `BudgetExceeded` and `KillSwitchActive`. Persist a `status="blocked"` experiment row with the breaker name embedded in the artifact list, and exit with a non-zero code.
5. Pass the returned `usage_proxy` fields into `insert_experiment` so future runs see them.
...
def run_next(slug: str, provider: str = typer.Option(..., "--provider")) -> None:
    ...
    # Build the governor seeded with the latest run's accumulated usage.
    ceilings = Phase0HardCeilings.from_env()
    store = _store()
    totals = store.get_run_usage_totals(slug)
    accumulators = RunAccumulators(
        provider_calls=int(totals["provider_calls"]),
        codex_calls=int(totals["codex_calls"]),
        claude_calls=int(totals["claude_calls"]),
    ...
    try:
        ...
        _persist_blocked_experiment(
            ...
            run_id=run_id,
            adapter=adapter,
            breaker_or_reason="KillSwitch",
            message=str(exc),
        )
        console.print(f"[red]task {packet['task_id']} blocked by kill switch[/red]")
...
def _persist_blocked_experiment(
    *,
    ...
    the first artifact path entry. PR4's event log will replace this with
    a structured event when it lands."""
    now = datetime.now(UTC).isoformat(timespec="seconds")
...
def test_run_next_halted_by_kill_switch_file(fixture_workspace: Path, monkeypatch) -> None:
    """Acceptance criterion #2: touching .arena/KILL_SWITCH halts in-flight
    tasks within one polling interval. For PR2 stubs, the polling interval
    is "before the next invoke", so we activate before run-next."""
    ...
    assert exp is not None
    assert exp["status"] == "blocked"
    assert "<blocked:KillSwitch>" in exp["artifact_paths"]
    store.close()
```
...
get_run_usage_totals(slug), wraps adapter.invoke in a Watchdog, and on
any cap violation (BudgetExceeded) or kill-switch activation
(KillSwitchActive) persists a status=blocked experiment row carrying the
breaker name in artifact_paths and exits with code 2.
...
- test_run_next_halted_by_kill_switch_file: touches the kill-switch file
  before run-next, asserts the task is blocked before any invoke.
...
arena run-next tabular_binary_v1 --provider stub_codex   # exits 2, blocked by kill switch
...
2. Activating the kill switch (file or env var) halts an in-flight `run-next` before the provider invokes.
3. `arena budget status` reports current accumulators against ceilings and shows kill switch state.
...
1. **Input/output char overflows map to `Breaker.PROVIDER_CALL` as a coarse proxy** because PR2 owns 5 named breakers and there is no dedicated input-chars/output-chars breaker. PR4 may introduce one if needed; the test acknowledges either.
2. **`KillSwitchActive` is a separate exception from `BudgetExceeded`** because the kill switch is a manual stop, not a cap violation. The CLI translates each into the right phase (`BLOCKED_KILL_SWITCH` vs `BLOCKED_BUDGET`) — though PR2 only updates `experiment.status` to `"blocked"` (the experiment-level enum doesn't distinguish; the breaker name lives in `artifact_paths` until PR4's event log).
3. **WasteDetector ships interface-only in PR2** — the live event observer that calls `observe_failed_command` lands in PR4 with the trace store. Unit tests exercise the methods directly.
4. **`BudgetGovernor.__init__` accepts a starting `accumulators`** so the CLI can seed it from the scoreboard. The governor itself is per-process and has no persistence; persistence lives in the scoreboard.
5. **`get_run_usage_totals` uses `LIKE 'codex%'`/`'stub_codex%'`** to count provider calls per family. This is brittle — a future provider name like `claude_codex_hybrid` would double-count. Acceptable for PR2 (only stub providers); PR7's real adapters should use a more explicit family field if the heuristic gets fragile.
6. **Kill switch polling = "before next invoke"** in PR2 because stubs are synchronous and fast. Real per-event polling during a long-running subprocess lands in PR7.
7. **Blocked experiment row carries the breaker name in `artifact_paths`** as a stopgap. PR4's event log will subsume this with a proper `breaker_triggered` event.
