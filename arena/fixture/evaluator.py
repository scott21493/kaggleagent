...
def evaluate_fixture_submission(
    submission_csv: str | Path, hidden_labels_csv: str | Path
    ...
    if list(submission.columns) != expected_columns:
        return FixtureEvaluationResult(
            False,
            None,
            error=f"expected columns {expected_columns}, got {list(submission.columns)}",
        )
    if list(labels.columns) != expected_columns:
        return FixtureEvaluationResult(
            False, None, error=f"hidden labels must have columns {expected_columns}"
        )
